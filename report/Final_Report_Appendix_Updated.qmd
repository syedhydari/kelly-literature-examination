...
This appendix presents brief literature context for Carta and Conversano's empirical Kelly template, followed by seven supporting figures referenced in the main report alongside the executable source code that produces each one. Section 2 provides the literature context. Section 3 sets up the global R environment. Sections 4–10 present Figures A1 through A7, one section each, with the figure rendered live from the code shown. Each figure is anchored to a specific subsection of the main report's results: A1 and A2 to §3.1 (Monte Carlo), A3 to §3.2 (Banca Intesa), A4 to §3.3 (in-sample EuroStoxx), A5 to §3.4 (long-only OOS), A6 to §3.5 (HAC inference), and A7 to §3.6 (unconstrained OOS). The pipeline downloads market data from Yahoo Finance via `tidyquant`/`quantmod` and runs all rolling-window backtests in-document.

# Literature Context  

**Carta and Conversano's Empirical Kelly Template**

The work of Carta and Conversano (2020) is a good fit for this project because it applies the Kelly Criterion to actual portfolio data. The authors move beyond the usual one-asset betting setup and demonstrate how Kelly can be used with estimated returns, covariance matrices, portfolio constraints, and rolling backtests. This makes their paper useful for studying what happens when a clean theory is tested on noisy market data. Notably, CC20 report that aggressive Kelly multiples deliver superior out-of-sample performance, but they do so without providing formal hypothesis tests or confidence intervals for those performance gaps. Their approach also connects Kelly to standard portfolio methods like minimum-variance and tangent portfolios. While this makes the method easier to implement, it also introduces practical issues; results can depend heavily on estimated returns, leverage choices, and specific portfolio constraints.

Our project uses their paper as both a replication target and a starting point for a closer review of the results. We maintain the basic structure of their backtests but focus more on the drivers of performance differences across Kelly variants. The primary question is whether the reported dominance of high-leverage Kelly multiples is a genuine strategic advantage or an artifact of implementation choices, such as the long-only budget constraints that may cause different Kelly variants to collapse onto an identical portfolio. This frames the broader issue behind the replication: while aggressive Kelly strategies may appear strong in a backtest, their reliability depends entirely on the underlying assumptions and constraints.
