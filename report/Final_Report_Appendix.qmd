...
Carta and Conversano (2020) are a good fit for this project because they apply the Kelly Criterion to actual portfolio data. They move beyond the usual one-asset betting setup and show how Kelly can be used with estimated returns, covariance matrices, portfolio constraints, and rolling backtests. This makes their paper useful for studying what happens when a clean theory is tested on noisy market data. Their approach also connects Kelly to standard portfolio methods like minimum-variance and tangent portfolios. This makes the method easier to implement, but it also creates practical issues. The results can depend heavily on estimated returns, leverage choices, and the constraints placed on the portfolio. Our project uses their paper as both a replication target and a starting point for a closer review of the results. We keep the basic structure of their backtests, but focus more on what is driving the performance differences across Kelly variants. The main question is whether those differences come from the Kelly strategy itself or from implementation choices such as long-only constraints, leverage limits, and how out-of-sample returns are compared. This helps frame the broader issue behind the replication. Aggressive Kelly strategies may look strong in a backtest, but their reliability depends on the assumptions and constraints built into the test.
